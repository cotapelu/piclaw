...
function createTodoTool(api: ExtensionAPI): ToolDefinition<any, TodoToolDetails> {
  ...
  return {
    ...
    promptGuidelines: [
      ...
      "Incorrect: todos({ operation: 'add_phase', params: {...} })",
      "",
      "Operations:",
      "• add_phase: { name: string (required), tasks?: [{ content: string, status?: string, notes?: string, details?: string }] }",
